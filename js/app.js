...
function compareUserEnteredResponse(question, validChoices) {
  ...
  while(attemptsAllowed <= 6) {
    ...
    if (!correctGuessFlg) {
      alert(wrongGuessMessage + ' You have ' + (6-attemptsAllowed) + ' attempts remaining.');
      nbrOfIncorrectGuesses++;
      attemptsAllowed++;
      console.log('attemptsConsumed:', attemptsAllowed);
    } else{
      break;
    }
...
function checkNumberGuess(question, userResponse, guessesAllowed) {
  ...
  while (userResponse !== 1 && guessesAllowed <= 3) {
    ...
    console.log('myBirthMonth guessed: ', userResponse);
